...
**Общий Redis для нескольких приложений.** Если несколько приложений с этой библиотекой делят один Redis (и одну базу), их сессии лежат в одном пространстве ключей: refresh-токен одного приложения будет принят другим. Разведите их опцией `WithKeyPrefix` (или отдельным `DB` у каждого приложения):
```go
redisStore := store.NewRedisStore(&redis.Options{
    Addr: "localhost:6379",
